...
app.get("/matches/:matchId/", async (request, response) => {
  ...
  const matchDetailsQuery = `
    ...
    WHERE 
      match_id = ${matchId};`;
  const player = await database.get(matchDetailsQuery);
...
app.get("matches/:matchId/players/".async(request,response)=>{
    const{ matchId }=request.params;
    const getMatchesPlayersQuery=`
      SELECT
         * 
       FROM 
         player_match_score
           NATURAL JOIN players_details 
      WHERE match_id= ${matchId};`;
    const playersArray=await database.all(getMatchesPlayersQuery);
